...
| `use_tcp` | `true` | 启动 `armv7_tcp`(`tcp` frame + `/armv7/payload` 发布) |
| `ee` | `` (空) | 末端执行器: `dummy_gripper` 走预置夹爪;或填一个绝对路径到 `*.urdf.xacro`(定义宏 `armv7_ee parent=...`) |
...
ros2 run armv7_examples pick_and_place      # 用 dummy gripper 抓 → 放 (需 ee=dummy_gripper)
```

### 末端执行器与传感器集成
```bash
# 干跑 + dummy 二指夹爪(同时拉起 ee_gripper_controller 这条链路)
ros2 launch armv7_bringup arm.launch.py use_fake_hardware:=true use_rt:=false \
     ee:=dummy_gripper

# RViz 里能看到末端多出夹爪几何 + ee_tcp 这个 frame
# 看 /armv7/payload(latched topic)
ros2 topic echo /armv7/payload --once

# 运行时改 TCP 偏移 / payload(立即生效)
ros2 param set /tcp_publisher_node tcp_offset_xyz "[0.0, 0.0, 0.20]"
ros2 param set /tcp_publisher_node payload_mass  0.5
```

- **力/扭矩传感器**:见 [docs/integration/ft_sensor.md](docs/integration/ft_sensor.md),覆盖 EtherCAT 串接(ATI Axia80-EC)和 网口 wrench topic(RobotIQ FT-300)两条路径。
- **手眼相机**:`armv7_eyehand` 提供 RealSense D435 的 mount xacro 与 `easy_handeye2` 输出的静态 TF launch,直接填入标定结果即用。
...
- ~~末端执行器 / 夹爪 / 传感器还未集成~~ → Phase 3 已完成:`ee=dummy_gripper` 直接可用;F/T 与相机集成有详细文档模板。
