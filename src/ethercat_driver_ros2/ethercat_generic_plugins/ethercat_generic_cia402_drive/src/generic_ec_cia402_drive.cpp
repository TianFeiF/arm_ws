...
void EcCiA402Drive::processData(size_t index, uint8_t * domain_address)
{
  ...
  if (pdo_channels_info_[index].index == CiA402D_RPDO_CONTROLWORD) {
    if (is_operational_) {
      ...
      if (auto_state_transitions_) {
        // Do NOT advance the CiA402 state machine past SWITCH_ON_DISABLED until
        // we have a valid position reading. Otherwise the drive can reach
        // OPERATION_ENABLED on the same cycle that 0x607a is still 0 (the
        // target-position seed lags actual feedback by one cycle), producing a
        // single-cycle "twitch" toward zero at OP entry. Holding the control
        // word at its default (0) keeps the drive disabled until 0x607a has
        // been seeded with the current pose for several cycles.
  ...
  // Seed the target-position (0x607a) default with the current actual
  // position so that, before any controller command arrives, the drive holds
  // its present pose instead of snapping to 0 once it enters OP.
  //
  // The original guard (mode_of_operation_display_ != MODE_NO_MODE) fails on
  // drives that only report a non-zero 0x6061 AFTER reaching OPERATION_ENABLED
  // (EYOU servos do this). In that window default_value stayed NaN, ec_update
  // wrote nothing, and 0x607a kept its power-on value of 0 -> the joint jumped
  // to zero. Gating on a valid position reading instead seeds 0x607a from the
  // first cycle that has real feedback, well before the CiA402 state machine
  // enables the drive.
